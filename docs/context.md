...
> Living context document. Captures _why_ this project exists, _who_ it
> serves, _what_ the target end state is, and _how we'll know we got
> there_. Code structure and workflow rules live in `README.md` and
...
- **Mobile / on-site (clubs, tournaments, customer location):**
  _possibly_ offered — **needs confirmation with the business owner**
...
1. **Persistence** — store bookings in a database (Supabase) instead of
   email-only. _(Prerequisite for everything below; implied by the
   booking ticket's own out-of-scope note.)_
...
1. **Mobile / on-site stringing** — is it actually offered? If so, it
   affects booking flow, pricing, and content. _(Confirm before
   building.)_
...
- Treat it as the source of truth for _product intent_. When a planned
